...
- `woocommerce_cart_calculate_fees` adds the configured COD fee for the selected plugin rate when COD is selected.
...
- `woocommerce_shipping_calculator_enable_postcode` hides the cart shipping-calculator postcode field because plugin rates use city/district.
- `woocommerce_package_rates` defensively removes stale non-heavy plugin rates when heavy sink shipping is active.
...
`WooCommerce → Simple Shipping Rule`

The per-zone WooCommerce shipping method still controls whether `Simple Smart Shipping` is enabled in a Saudi shipping zone. The dedicated settings page controls rates, thresholds, city/district aliases, COD settings, labels, estimates, and debug logging.
...
The plugin owns COD availability and fee logic. Disable old custom snippets that add COD fees or hide COD for these same plugin rates, otherwise WooCommerce may show duplicate or conflicting behavior. WooCommerce recalculates fees when the selected payment/shipping method changes; the frontend script debounces payment-method refreshes to one checkout update.
